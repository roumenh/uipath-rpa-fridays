...
<html>
    ...
    <body>
        ...
        <?php if ($_REQUEST["page"] == "send_request"){ ?>
            ...
            Text to convert: <textarea name="text" cols="50" rows="5"></textarea><br>
        ...
        <?php }else{ ?>
            <h1>Home</h1>
            <?php
                $sql = "SELECT id,status,text,morse FROM requests ORDER BY id DESC";
                $query = mysqli_query($conn,$sql);
                while ($row = mysqli_fetch_array($query)){
                    echo $row["id"]." | ".$row["status"]." | ".$row["text"]." | ".$row["morse"]." |<br>";
                }
            ?>
